...
SE is zero when all completions fall into a single cluster and is maximized when they spread evenly across many clusters. Sharma _et al._ @sharma2025assessingcorrectnessllmbasedcode shows that CC and SE produce comparable results when clustering is accurate. Both are reported for direct comparison.

Sharma _et al._ @sharma2025assessingcorrectnessllmbasedcode also propose Mutual Information (MI), which queries the model twice per problem, with the second prompt formed by appending the first response to the original prompt. MI is excluded here because it produces structurally different completions, which would make pass\@1 incomparable across methods.

The two execution-based methods use the same CC and SE formulas, differing only in how equivalence is determined.

*Functional Clustering*

Proposed by @Ravuri2025EliminatingHE, determines equivalence by running each completion on concrete test inputs and grouping completions that produce identical outputs on all inputs. Test inputs are generated by prompting the LLM with only the function signature, making the method self-contained and applicable to benchmarks without ground-truth tests.
...
All methods share the same greedy completions, so pass\@1 is identical across all thirteen scores for a given model. It demonstrates the model's capability and can serve as a baseline for uncertainty estimation performance.

*Prediction Rejection Ratio (PRR)* @malinin2017prr measures how well uncertainty scores identify incorrect predictions. Problems are ranked by decreasing uncertainty and progressively removed. At each threshold, accuracy is computed on the remaining ones. PRR is the area between this curve and the random-rejection baseline (a horizontal line at pass\@1), normalized by the area between the oracle curve (which rejects all incorrect predictions first) and the same baseline:
...
#figure(
  image("../../figures/prr.png", width: 80%),
